...
fanshoutiaoTestScreenView::fanshoutiaoTestScreenView()
    : waveformLen(0), animIndex(0), animating(false),
    swingCount(0), lastPeak(0), lastScore(0)
{
}

void fanshoutiaoTestScreenView::setupScreen()
{
    fanshoutiaoTestScreenViewBase::setupScreen();
...
void fanshoutiaoTestScreenView::tearDownScreen()
{
    fanshoutiaoTestScreenViewBase::tearDownScreen();
}
...
void fanshoutiaoTestScreenView::onSwingResult(const SwingResult_t& res)
{
...
void fanshoutiaoTestScreenView::handleTickEvent()
{
    if (!animating) return;
...
void fanshoutiaoTestScreenView::onStopClicked()
{
    presenter->onStopTraining();
}

void fanshoutiaoTestScreenView::onTrainingDone(const TrainingDone_t& done) {
    animating = false;

    presenter->saveTrainingResult(done, bestSwing);
    application().gotofanshoutiaoSummaryScreenScreenWipeTransitionWest();
}