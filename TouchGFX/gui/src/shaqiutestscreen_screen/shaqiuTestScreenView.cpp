...
shaqiuTestScreenView::shaqiuTestScreenView()
    : waveformLen(0), animIndex(0), animating(false),
    swingCount(0), lastPeak(0), lastScore(0)
{
}

void shaqiuTestScreenView::setupScreen()
{
    shaqiuTestScreenViewBase::setupScreen();
...
void shaqiuTestScreenView::tearDownScreen()
{
    shaqiuTestScreenViewBase::tearDownScreen();
}
...
void shaqiuTestScreenView::onSwingResult(const SwingResult_t& res)
{
...
void shaqiuTestScreenView::handleTickEvent()
{
    if (!animating) return;
...
void shaqiuTestScreenView::onStopClicked()
{
    presenter->onStopTraining();
}

void shaqiuTestScreenView::onTrainingDone(const TrainingDone_t& done) {
    animating = false;

    presenter->saveTrainingResult(done, bestSwing);
    application().gotoshaqiuSummaryScreenScreenWipeTransitionWest();
}