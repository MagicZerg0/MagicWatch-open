...
#include "FreeRTOS.h"       // ← 加
#include "task.h"           // ← 加
#include "semphr.h"         // ← 加
extern SemaphoreHandle_t g_wakeupSemaphore;   // ← 加
...
void EXTI0_IRQHandler(void)
{
  ...
  if (g_wakeupSemaphore != NULL)
  {
      xSemaphoreGiveFromISR(g_wakeupSemaphore, &xHigherPriorityTaskWoken);
  }
...
void EXTI15_IRQHandler(void)
{
  ...
  HAL_GPIO_EXTI_IRQHandler(CST816_INT_Pin);
